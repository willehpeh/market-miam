...
Next: **Step 4c** — `apps/api` module wiring (4a NOTIFY-migration, 4b `PostgresNotifications`, and 4d migrate-on-boot are done; 4d was moved *ahead* of 4c so every commit stays bootable). (`version`, 1a, 1b, 2, 3 committed — see the Steps checklist.) Runner note: `ConsumerRunner` has been renamed `Subscriptions`.
...
- [ ] 4 — LISTEN/NOTIFY + wiring. Sub-commits **reordered (d) before (c)** so every commit stays bootable; **(a), (b), (d) done, (c) is next**:
