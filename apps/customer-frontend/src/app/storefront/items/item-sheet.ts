...
@Component({
  ...
  styles: `
    ...
    .item-sheet[open]::backdrop {
      background-color: rgb(0 0 0 / 0.5);
    }
    .item-sheet.closing::backdrop {
      background-color: rgb(0 0 0 / 0);
    }
    @starting-style {
      .item-sheet[open]::backdrop {
...
export class ItemSheet {
  protected readonly item = signal<ItemViewModel | null>(null);
  protected readonly dragOffset = signal<number | null>(null);
  protected readonly closing = signal(false);
  private readonly dialog = viewChild.required<ElementRef<HTMLDialogElement>>('dialog');

  open(item: ItemViewModel): void {
    this.item.set(item);
    this.dialog().nativeElement.showModal();
  }

  // The dialog element survives between items, and an <img> keeps its old pixels on a src
  // change until the new photo decodes. Tearing the content down on close means the next
  // open builds a fresh <img> and a fresh scroller — no previous item's photo while the new
