...
import { LoadMarketPrices, marketPricesFeature, selectPricesFor, SetMarketPrices } from './market-prices.state';
...
export class StoreMarketPricesFacade implements MarketPricesFacade {
  ...
  pricesFor(marketId: string): Signal<PriceList> {
    return this.store.selectSignal(selectPricesFor(marketId));
  }
