...
type Row = { name: string; description: string; phone: string; image_reference: string };
...
export class PostgresVendorStorefrontViews implements VendorStorefrontViews, VendorStorefrontViewStore {
  ...
  async findByVendor(vendorId: string): Promise<VendorStorefrontView | undefined> {
    const { rows } = await this.db.query<Row>(
      'SELECT name, description, phone, image_reference FROM vendor_storefront_views WHERE vendor_id = $1',
      [vendorId],
    );
    const [row] = rows;
    return row
      ? { name: row.name, description: row.description, phone: row.phone, imageReference: row.image_reference }
      : undefined;
  }
