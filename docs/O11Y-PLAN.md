...
## Shipped
Producer steps 1–3 (command-dispatch span; event-store append/load spans; `traceparent` into event metadata) and consumer steps 5–6 (new-trace-per-handler + span link to producer + `processing.lag_ms`). The consumer handle span is emitted by `PollingSubscription` itself for every discovered projection and processor (inlined per ADR 0044; originally a `TracingEventHandler` wrapper) — uniform, no per-projection wiring. First processor→command fan-out (`StorefrontOpener`): the dispatched command and its appended event run on the opener's own consumer trace (child of the handle span), linked back to the request rather than threading the request trace through. Commits `1964004`, `bc3f826`, `8fc68e1`, `efbb5ab`, `662b08c`, `d2e4599`, `3b2e26b`, `696ba33`, `e481fa2`, `8115c96`, `944c49f`, `74acc02`; `*-tracing.spec.ts` in `apps/api`.
...
4. **Per-type intent/outcome attributes** — adapter-side per-type extractors in `apps/api` (allow-list per command/event type; exact-match-tested = PII guard). Dispatch span ← command; append span ← events. No handler span (handlers in `packages/market-days` stay OTel- and telemetry-free).
   ...
   **Started on the command side.** `SetMarketDayMenu` gained its entry point (`PUT /market-days/:marketId/:date/menu`, menu du jour slice 4) and ships `menu.item_count` with it, taken ahead of the value gate: the dish count per day is the baseline the waste-watch feature measures against, so it is wanted from the pilot's first day rather than retrofitted over an empty history.

   The rest stay deferred on value, not readiness. `AddItemToCatalogue` is dispatched in production (`POST /api/catalogue`, `CatalogueController`) carrying a safe aggregatable `price` (integer cents), but with no real vendors an `item.price` attribute aggregates over nothing, and the dispatch span is already payload-blind so there is no leak to fix. `EditStorefrontInformation` (`PUT /storefront`) has an entry point but an all-free-text payload (`name`, `description`) — nothing safe beyond `vendor.id`, already on the span. Do those when launch brings real catalogue writes worth aggregating (`item.price`, `item.has_image`).

   **Design — command half built, event half still on paper.** `apps/api/src/app/event-sourcing/command-attributes.ts` exists and is spread into `tracing/command-gateway.ts`; `event-attributes.ts` is not built, since nothing yet instruments an event. Each is a `Record<string, (msg) => Record<string, AttributeValue>>`, default empty, opt-in per type (`registry[key]?.(msg)` → missing key adds nothing, baseline unchanged). Spread the lookup into the existing `setAttributes` in `tracing/command-gateway.ts` (key `command.constructor.name`) and the append span in `packages/event-sourcing/src/adapters/application.event-store.ts` (key `events[0].type`, read `events[0].payload`). May import domain types from `@market-miam/market-days`. (`@opentelemetry/*` in `packages/event-sourcing` is now the norm — ADR 0044 — but the *extractors* stay in `apps/api`: they encode app-specific attribute policy, and the domain package `market-days` stays telemetry-free.) Rules: raw value OK for non-PII public data (`item.price` raw — list price, not sensitive revenue); free-text → derived scalars only (`*.has_image`, `schedule.day_count`, `menu.item_count`), never raw text. Guard: extend the exact-match `toEqual({...})` assertions in `tracing/command-gateway.spec.ts` (the file this plan previously called `command-dispatch-tracing.spec.ts`); each instrumented command gets a social test through its real production entry point (no entry point ⇒ not ready). Those exact-match assertions are what keeps the opt-in honest — adding an entry for an uninstrumented command breaks them, so a leak cannot land quietly. Optional later: ESLint `no-restricted-imports` banning `@opentelemetry/*` in `packages/market-days` (the domain package; `packages/event-sourcing` legitimately imports it per ADR 0044 — matches the `projection-decorator` / `processor-decorator` custom-rule pattern; today the invariant is structural/ADR only).
...
## Deferred (per ADR)
- OTel Collector + tail-based sampling — until volume warrants (tracked in the root `NEXT_BEHAVIOURS.md`).
