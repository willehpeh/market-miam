...
- **Decoration order:** `ApplicationEventStore` composes the chain in its constructor — `Tracing(Lineage(Shredding(leaf)))`, shredding closest to the store. Both `EventStore` and `Events` resolve through the one instance (Tracing/Lineage widened to implement `Events` so `loadFrom` decrypts too). Identical in both profiles.
