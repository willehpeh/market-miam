...
- **`Catalogue.apply` rehydration bug (untouched):** only `ItemAddedToCatalogue` is applied on reload, not `ItemPriceChanged`/`ItemRetired`. Off the add/list path; fix before building price-change/retire endpoints.
