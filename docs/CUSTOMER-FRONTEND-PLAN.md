...
14. **Vendor publish button + storefront URL. ✅ Shipped (A–D) — see below.**

**Step 14 — vendor publish button + URL (resolved via grill, 2026-07-18). ✅ Shipped (sub-slices A–D).** Wire the vendor-frontend "Publier" button to `POST /storefront/publish` and show the vendor their storefront URL. (The post-publish vendor-app behaviour change remains a separate follow-up — see below.)
...
- **A — reverse read + `url` gate. ✅ Shipped.** `subdomainFor(vendorId)` on the `SubdomainRegistry` port + in-memory/pg adapters (in-memory spec + pg container spec). Publish handler injects `SubdomainRegistry`, reads the subdomain, passes `hasSubdomain` into `StorefrontPublication.publish(...)` (service stays pure); absent → `url` in `missing[]` (appended last). New `publish-storefront.spec` cycle + full `missing[]` now `[…, 'url']`; ready tests (social + `storefront-publish.spec`) seed a subdomain; api acceptance gained a no-subdomain → 400 `url` case. **Ripple fixed:** `dev-seed` now registers the demo subdomain *before* `PublishStorefront` (publish requires it). Pg container spec (`test:container`) passes against a real Postgres — 7 tests.
- **B — expose subdomain on the vendor read. ✅ Shipped.** `FindVendorStorefront` returns `VendorStorefront = VendorStorefrontView & { subdomain: string | null }`; the handler injects `SubdomainRegistry` and composes the subdomain at read time (`null` when unassigned) — the stored read model is untouched. `GET /storefront` acceptance test gained `subdomain: null` + an assigned-subdomain case. **Ripple:** the rebuild + erasure full-body assertions gained `subdomain: null` (erasure deletes the row → `null`, nicely confirming it).
- **C — frontend publish flow. ✅ Shipped.** The port→effect→facade chain (`Storefront.publish()` → `POST /api/storefront/publish`; `PublishStorefront`/`Success`/`Failure` actions + `publishing`/`published`/`publishError` state; `publishStorefront$` effect mirroring `editStorefront$`; facade `publish()` + signals; fake facade updated). Social spec extended (sends POST, publishing flag, published on 204, publishError on 400). `PublishStorefrontFailure` carries no reasons — the fallback is generic (checklist shows specifics).
- **D — Dashboard wiring + URL line. ✅ Shipped.** Added `storefrontBaseDomain` to the vendor env (all 3 files) + `subdomain` to the frontend `StorefrontView`. The Dashboard wires the "Publier" button to `facade.publish()` with loading (`Publication…`, disabled) / confirmation (`✓ Vitrine publiée`, button hidden) / error states, and shows the always-a-link URL line ("Votre vitrine sera publiée à → `{subdomain}.marketmiam.fr`"). **Deviation from the grill:** rather than a literal 4th checklist *row* (the subdomain isn't vendor-actionable — no link to "fix" it), the publish block gates on the subdomain: when it's missing the button is replaced by "Votre adresse web est en cours d'attribution." The button is still gated on subdomain presence, matching the intent. Dashboard spec extended (URL link, publish click, publishing/published/error, no-subdomain-pending); several `StorefrontView` fixtures across specs gained `subdomain`.
...
- **Vendor publish button + URL (step 14). ✅ Shipped (A–D).** Subdomain is a publish requirement (`subdomainFor` reverse read), the Dashboard gates + wires the "Publier" button, and shows the always-a-link URL. **Remaining follow-up:** the post-publish vendor-app behaviour change (screen swaps once published) — see `NEXT_BEHAVIOURS.md`.
